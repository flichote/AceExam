...
| 任务 | 卡片 ID | 备注 |
|---|---|---|
| T28 | t_???????? | 本任务（架构增量 + API 契约 + 任务图），done 后解锁 T29/T30/T32 |
| T29 | t_???????? | 待 T28 done 自动解锁 |
| T30 | t_???????? | 待 T28 done 自动解锁 |
| T31 | t_???????? | 待 T30 done 自动解锁 |
| T32 | t_???????? | 待 T28 done 自动解锁 |
| T33 | t_???????? | 待 T30+T31+T32 done 自动解锁 |
